...
- Stripping off `ci/prow/` prefixes to run `/test` or `/retest`
...
Assume your target repo is `org/repo`.
...
- Give it your approval and LGTM:
  ...
  ```bash
  autoprat -r org/repo --lgtm --approve 123
  ```

- Grant ok-to-test:

  ```bash
  autoprat -r org/repo --ok-to-test 123
  ```

- Re-run just the `test-fmt` job:

  ```bash
  autoprat -r org/repo 123 -j ci/prow/test-fmt
  # posts: /test test-fmt
  ```
...
- Override a context:
  ...
  ```bash
  autoprat -r org/repo 123 -x /override -j ci/prow/test-fmt
...
- Post a bare `/retest`:
  ...
  ```bash
  autoprat -r org/repo --retest 123
  # posts: /retest
  ```
...
- Approve, LGTM and OK-to-test every open PR:
  ...
  ```bash
  autoprat -r org/repo -a --lgtm --approve --ok-to-test
  ```

- Re-run two jobs on every broken PR and leave a note:

  ```bash
  autoprat -r org/repo -a \
    -j ci/prow/test-fmt \
    -j ci/prow/security \
    -x /retest \
...
```
  ...
  --lgtm                    Add /lgtm (if no 'lgtm' label)
  --approve                 Add /approve (if no 'approved' label)
  --ok-to-test              Add /ok-to-test (if no 'ok-to-test' label)
  -c, --comment TEXT        Add a custom comment (always posted)
  -j, --job JOB_NAME        If JOB_NAME failed, post "<cmd> <basename-of-job>"
                            (repeatable)
  -x, --cmd PREFIX          Prefix for job comments (default: /test)
  --retest                  Shortcut for `-c /retest`
  -n, --dry-run             Show what would be done without posting
  -t, --show-title          Display PR titles in output
